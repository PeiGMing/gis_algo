...
export class MapComponent implements OnInit, OnDestroy {
  ...
  private initDatGUI() {
    ...
    this.gui.add(this.options, 'UpAndDown').min(-1).max(1).step(0.05).listen().onFinishChange((value) => {
      this.service.up_and_down = value;
    });
    this.gui.add(this.options, 'LeftAndRight').min(-1).max(1).step(0.05).listen().onFinishChange((value) => {
      this.service.left_and_right = value;
    });
    this.gui.add(this.options, 'Rotate').min(-180).max(180).step(5).listen().onFinishChange((value) => {
      this.service.rotate = value;
    });
    this.gui.add(this.options, 'ScaleX').min(0.3).max(3).step(0.1).listen().onFinishChange((value) => {
      this.service.scaleX = value;
    });
    this.gui.add(this.options, 'ScaleY').min(0.3).max(3).step(0.1).listen().onFinishChange((value) => {
